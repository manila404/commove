        ...
        const unsub = subscribeToNotifications(
            ...
            (err) => {
                if (err?.code === 'permission-denied') {
                    setNotificationError("We couldn't load your notifications due to a permissions issue. Please contact support.");
                } else {
                    setNotificationError("We couldn't load your notifications right now. Please try again later.");
                }
            }
        );
        return () => unsub();
    ...
    if (notificationError) {
        return (
            <div className="flex flex-col items-center justify-center py-20 text-center px-4">
                <div className="w-16 h-16 bg-red-50 dark:bg-red-900/20 rounded-full flex items-center justify-center mb-4 border border-red-100 dark:border-red-800">
                ...
                <p className="text-gray-500 dark:text-gray-400 text-sm max-w-sm">{notificationError}</p>
            </div>
        );
    }
