...
const ToggleSwitch = ({ label, description, checked, onChange }: {
  ...
  <div className="flex items-center justify-between py-4 border-b border-gray-100 dark:border-gray-800 last:border-0">
    ...
    <button onClick={onChange} className={`w-12 h-6 rounded-full p-1 transition-colors duration-300 ease-in-out ${checked ? 'bg-primary-600' : 'bg-gray-200 dark:bg-gray-700'}`}>
...
const NotificationSettingsView: React.FC<NotificationSettingsViewProps> = ({ currentUser, onBack }) => {
  ...
  const updateSettings = async (newSettings: any) => {
    ...
    if (currentUser) {
      try { await updateUserNotificationSettings(currentUser.uid, newSettings); }
  ...
  const toggle = async (key: string) => {
    ...
    if (key === 'vibrationEnabled' && newEnabled) {
      if (isInWebView()) {
        postToNative({ type: 'VIBRATE', pattern: [0, 200] });
  ...
  const handleSendTestNotification = async () => {
    ...
    setTimeout(() => {
      if (isInWebView()) {
        // APK: fire a real system notification via the native bridge
        postToNative({
          type: 'SEND_NOTIFICATION',
          title: 'Commove Test Notification',
          body: 'This is a test notification from Commove. Your settings are working correctly!',
        });
      } else if ('Notification' in window && permissions.notifications === 'granted') {
        // Desktop browser notification
        try { new Notification('Commove Test Notification', { body: 'This is a test notification from Commove. Your settings are working correctly!' }); }
  ...
  const handleResetToDefaults = () => {
    updateSettings({
      pushEnabled: true, emailEnabled: false, upcomingReminders: true, newEvents: false,
      quietHoursEnabled: false, quietHoursStart: '22:00', quietHoursEnd: '07:00',
      emailFrequency: 'instant' as const, vibrationEnabled: true,
    });
  };
    ...
    <div className="min-h-screen bg-white dark:bg-gray-900 transition-colors duration-300 pb-20">
      <main className="container mx-auto p-4 md:p-6 animate-fade-in-up">
        <div className="flex items-center justify-end mb-6">
          <button onClick={handleResetToDefaults} className="text-xs font-bold text-gray-400 hover:text-red-500 transition-colors uppercase tracking-widest">
            Reset Defaults
          </button>
        </div>
        ...
        <div className="bg-white dark:bg-gray-800 rounded-2xl p-6 shadow-sm border border-gray-100 dark:border-gray-700 mb-6">
          ...
          <ToggleSwitch label="Push Notifications" description="Receive alerts on this device" checked={settings.pushEnabled} onChange={() => toggle('pushEnabled')} />
          {settings.pushEnabled && (
            <div className="ml-4 pl-4 border-l-2 border-gray-100 dark:border-gray-800 mb-4">
              <ToggleSwitch label="Vibration" description="Haptic feedback for alerts" checked={settings.vibrationEnabled || false} onChange={() => toggle('vibrationEnabled')} />
              <button onClick={handleSendTestNotification} disabled={isTesting} className="mt-2 text-xs font-bold text-primary-600 dark:text-primary-400 flex items-center gap-2 hover:underline disabled:opacity-50">
                {isTesting ? (<><div className="w-3 h-3 border-2 border-primary-600 border-t-transparent rounded-full animate-spin" />Sending Test...</>) : ('Send Test Notification')}
              </button>
            </div>
          )}
        ...
        <div className="bg-white dark:bg-gray-800 rounded-2xl p-6 shadow-sm border border-gray-100 dark:border-gray-700 mb-6">
          ...
          <ToggleSwitch label="Enable Quiet Hours" checked={settings.quietHoursEnabled || false} onChange={() => toggle('quietHoursEnabled')} />
          {settings.quietHoursEnabled && (
            <div className="grid grid-cols-2 gap-4 mt-4">
              <div>
                <label className="block text-[10px] font-bold text-gray-400 uppercase mb-1">Start Time</label>
                <input type="time" value={settings.quietHoursStart} onChange={(e) => handleInputChange('quietHoursStart', e.target.value)} className="w-full bg-gray-50 dark:bg-gray-900 border border-gray-200 dark:border-gray-700 rounded-lg p-2 text-sm text-gray-900 dark:text-white" />
              </div>
              <div>
                <label className="block text-[10px] font-bold text-gray-400 uppercase mb-1">End Time</label>
                <input type="time" value={settings.quietHoursEnd} onChange={(e) => handleInputChange('quietHoursEnd', e.target.value)} className="w-full bg-gray-50 dark:bg-gray-900 border border-gray-200 dark:border-gray-700 rounded-lg p-2 text-sm text-gray-900 dark:text-white" />
              </div>
            </div>
          )}
        </div>
        ...
        <div className="bg-white dark:bg-gray-800 rounded-2xl p-6 shadow-sm border border-gray-100 dark:border-gray-700">
          ...
          <ToggleSwitch label="Upcoming Reminders" description="Get notified before saved events" checked={settings.upcomingReminders} onChange={() => toggle('upcomingReminders')} />
          <ToggleSwitch label="New Events in Bacoor" description="Alerts when new events are published" checked={settings.newEvents} onChange={() => toggle('newEvents')} />
        </div>

        <div className="mt-8 text-center">
          <p className="text-[10px] text-gray-400 dark:text-gray-500 uppercase tracking-[0.2em]">Commove Notification Engine v2.1</p>
        </div>
      </main>
    </div>
  );
